...
Elm.NaiveBayes = Elm.NaiveBayes || {};
Elm.NaiveBayes.make = function (_elm) {
   "use strict";
   _elm.NaiveBayes = _elm.NaiveBayes || {};
   if (_elm.NaiveBayes.values) return _elm.NaiveBayes.values;
   var _U = Elm.Native.Utils.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var getClassValues = F2(function (a,model) {
      var allValues = A2($List.map,function (n) {    return n.values;},model.data);
      var arrayOfTransposition = $Array.fromList($List$Extra.transpose(allValues));
      var index = A2($List$Extra.elemIndex,a,model.classes);
      var _p0 = index;
      if (_p0.ctor === "Nothing") {
            return $Maybe.Nothing;
         } else {
            return A2($Array.get,_p0._0,arrayOfTransposition);
         }
   });
   var getFullDenominator = function (data) {
      var allValues = A2($List.map,function (n) {    return n.values;},data);
      var intercalation = A2($List$Extra.intercalate,_U.list([]),allValues);
      return $List.sum(intercalation);
   };
   var probabilityOfA = F2(function (a,model) {
      var data = model.data;
      var denominator = getFullDenominator(data);
      var filtered = $List.head(A2($List.filter,function (n) {    return _U.eq(n.name,a);},data));
      var sum = function () {    var _p1 = filtered;if (_p1.ctor === "Nothing") {    return 0;} else {    return $List.sum(_p1._0.values);}}();
      return $Basics.toFloat(sum) / $Basics.toFloat(denominator);
   });
   var probabilityOfB = F2(function (a,model) {
      var values = A2(getClassValues,a,model);
      var sum = function () {    var _p2 = values;if (_p2.ctor === "Nothing") {    return 0;} else {    return $List.sum(_p2._0);}}();
      var data = model.data;
      var denominator = getFullDenominator(data);
      return $Basics.toFloat(sum) / $Basics.toFloat(denominator);
   });
   var setData$ = F3(function (classes,data,model) {    return _U.update(model,{classes: classes,data: data});});
   var newDataEntry = F2(function (name,values) {    return {name: name,values: values};});
   var initialModel = {classes: _U.list(["test1","test2"]),data: _U.list([A2(newDataEntry,"john",_U.list([1,2])),A2(newDataEntry,"suzie",_U.list([3,4]))])};
   var setData = F2(function (classes,data) {    return A3(setData$,classes,data,initialModel);});
   var main = $Graphics$Element.show($Basics.toString(A2(probabilityOfB,"test2",initialModel)));
   var DataEntry = F2(function (a,b) {    return {name: a,values: b};});
   var Model = F2(function (a,b) {    return {classes: a,data: b};});
   return _elm.NaiveBayes.values = {_op: _op
                                   ,Model: Model
                                   ,DataEntry: DataEntry
                                   ...
                                   ,setData$: setData$
                                   ,setData: setData
                                   ,getFullDenominator: getFullDenominator
